...
// What create() refuses with: a status for the route to answer with, and for
// a duplicate, the id of the link that is already there so the client can
// show it instead.
class LinkError extends Error {
  /** @param {number} status @param {string} message @param {string} [existingId] */
  constructor(status, message, existingId) {
    super(message);
    this.status = status;
 ...
 * @property {string | undefined} icon   undefined when never asked, "" when asked and none
 * @property {string} note
 * @property {string[]} groups
 * @property {boolean} fetched
 * @property {string | null} fetchError
...
/** @param {any} row @returns {Link | null} */
function toLink(row) {
  if (!row) {
    return null;
  }
...
class LinkStore {
  ...
  async setIcons(answers) {
    ...
    // .immediate: this reads each link before writing it, and a transaction
    // that reads first has to take the write lock first or another process
    // committing in between leaves it holding a stale snapshot. See the same
    // note on mutateOrganizerState.
  ...
  async create(preview, { createdBy = null, note = "", groups = [] } = {}) {
    if (this.db.prepare("SELECT COUNT(*) AS n FROM links").get().n >= MAX_LINKS) {
      throw new LinkError(400, `This library holds at most ${MAX_LINKS} links.`);
    }

    const existing = this.findByUrl(preview.url);
    if (existing) {
      throw new LinkError(409, "That link is already saved.", existing.id);
    }
  ...
  /** Fields to change, all of them optional: what a person typed, or what a
   * refresh read off the page again. undefined anywhere means "leave that
   * alone", which is what lets a refresh carry new metadata without touching
   * the groups the link was filed under.
   *
   * @param {string} id
   * @param {Partial<import("./link-preview").Preview> & Partial<Link>} changes
   */
...
/* A link as a client sees it: the same record, with the icon as an address
 * rather than as the bytes. Shared by the REST route and the GraphQL view so
 * the two cannot describe a link differently.
 *
 * `icon` is left exactly as stored when it is undefined — never fetched — and
 * the client tells that apart from "" (fetched, and the site had none) to know
 * what still needs asking.
 */
const iconTags = new Map();

// A short tag for the icon's bytes, for the URL and the ETag. Cached per link
// so a list of two hundred is not two hundred hashes of a data URI each time.

...
function postDocsDelete(deps) {
  const { markdownDir, shareStore, fileExists, moveDocToRecycle, paramDocPath } = deps;
  ...
  try {
    ...
    const mode = readBody(req, { mode: { type: "string", default: "", trim: true } }).mode.toLowerCase();
...
function getRecycleBin(deps) {
  const { readOrganizerState, getRecycleDocs } = deps;
...
function getRecycleBinContent(deps) {
  const { softDeletedDir, fileExists, readCachedTextFile, paramEntryPath } = deps;
...
function postRecycleBinRestore(deps) {
  const { softDeletedDir, restoreFromBin, paramEntryPath } = deps;
...
function postRecycleBinHardDelete(deps) {
  const { softDeletedDir, hardDeletedDir, fileExists, moveFile, ensureUniqueFilenameInDir, invalidateCachedContent, paramEntryPath } = deps;
...
function getArchive(deps) {
  const { hardDeletedDir, readOrganizerState, getRecycleDocs } = deps;
...
function getArchiveContent(deps) {
  const { hardDeletedDir, fileExists, readCachedTextFile, paramEntryPath } = deps;
...
function postArchiveRestore(deps) {
  const { hardDeletedDir, restoreFromBin, paramEntryPath } = deps;
...
function deleteArchive(deps) {
  const { hardDeletedDir, fileExists, invalidateCachedContent, paramEntryPath } = deps;
  // Erasing is the one thing here that cannot be undone, so it is the one that
  // is written down; a caller that wired in no log gets the quiet one.
  const audit = deps.audit || noAudit;
  ...
  try {
    ...
    const confirmation = readBody(req, { confirmFile: { type: "string", default: "", trim: true } }).confirmFile;
    ...
    // The one operation in the app that cannot be undone, which is why it
    // needs the file name typed back and why it is worth a line saying who.
    audit("doc.erased", { ...actorOf(req), file: originalFile, entry: entryName });
 ...
 */
function createRecycleRoutes(deps) {
  const { requireRead, requirePermission } = deps;
  const router = express.Router();

  router.post("/api/docs/*file/delete", requirePermission("doc:write"), postDocsDelete(deps));

  router.get("/api/recycle-bin", requireRead, getRecycleBin(deps));

  router.get("/api/recycle-bin/*entry/content", requireRead, getRecycleBinContent(deps));

  router.post("/api/recycle-bin/*entry/restore", requirePermission("doc:write"), postRecycleBinRestore(deps));

  router.post("/api/recycle-bin/*entry/hard-delete", requirePermission("doc:write"), postRecycleBinHardDelete(deps));
  ...
  router.get("/api/archive", requireRead, getArchive(deps));

  router.get("/api/archive/*entry/content", requireRead, getArchiveContent(deps));

  router.post("/api/archive/*entry/restore", requirePermission("doc:write"), postArchiveRestore(deps));

  router.delete("/api/archive/*entry", requirePermission("doc:erase"), deleteArchive(deps));

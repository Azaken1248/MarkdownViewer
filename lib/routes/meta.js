 ...
 * expect, /healthz proves the storage the app depends on is readable — which
 * is the failure that matters, unlike a constant "ok" that only proves the
 * process is up — and /metrics is what this process has done, for a scraper
 * that holds the token it is turned on with.
 */
...
const graphQLSchema = buildSchema(`
  ...
  type Query {
    ...
    folders: [Folder!]!
    """Full-text search: the same words, the same order, the same snippets as
    the search box. The server has a ceiling of its own, and limit cannot raise
    the answer above it — asking for more than it holds returns what there is
...
function graphQLRootValue(deps) {
  ...
  search: async ({ query, limit }) => {
    /* Asked for down there rather than sliced up here.
     *
     * The search has a ceiling of its own, below the one this field
     * advertises, so slicing afterwards meant a caller asking for a thousand
     * got two hundred and nothing said which of the two numbers was real.
     * Passing it down makes the answer the smaller of the two, which is what
     * it always was, and the schema says so.
     */
    const { matches } = await searchDocuments(query, "docs", clampLimit(limit));
...
function createMetaRoutes(deps) {
  const {
    buildEmbedMeta, getDocs, requireRead, enableIntrospection,
    metrics = null, metricsToken = "", cacheStats = () => ({})
  } = deps;
  ...
  /* What this process has done, for a scraper.
   *
   * Off unless METRICS_TOKEN is set, and then it wants that token: a scrape
   * says how many requests failed, how long they took and how much of the
   * library is in memory, which is more than an anonymous caller has any
   * business knowing about a private library. There is no "allow from this
   * network" here to get wrong — either a caller holds the token or it does
   * not.
   *
   * The comparison is timing-safe because the alternative is a token somebody
   * can find one character at a time, and this is four lines.
   */
  router.get("/metrics", (req, res) => {
    if (!metrics || !metricsToken) {
      res.status(404).json({ error: "Metrics are not enabled." });
      return;
    }

    const offered = String(req.get("authorization") || "").replace(/^Bearer\s+/i, "");
    const wanted = Buffer.from(metricsToken);
    const given = Buffer.from(offered);

    if (given.length !== wanted.length || !crypto.timingSafeEqual(given, wanted)) {
      res.set("WWW-Authenticate", "Bearer");
      res.status(401).json({ error: "Metrics need their token." });
      return;
    }

    res.set("Content-Type", "text/plain; version=0.0.4; charset=utf-8");
    res.send(metrics.render({ cacheStats }));
  });

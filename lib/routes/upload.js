...
/* The `paths` field: a JSON array, index-aligned with the files.
 *
 * A multipart field, so a string carrying JSON rather than JSON. Answers the
 * list, or null when it is missing, unparseable, or does not line up with what
 * was actually sent — all of which mean the same thing to the caller.
 */
function readUploadPaths(req, files) {
  let paths = null;

  try {
    paths = JSON.parse(readBody(req, { paths: { type: "string", default: "[]" } }).paths);
  } catch {
    return null;
  }

  return Array.isArray(paths) && paths.length === files.length ? paths : null;
}

/* What is going where, before anything is written.
 *
 * One bad path entry costs that file its folder placement rather than the
 * whole upload — it falls back to the part's own filename — and a name this
 * app had to adjust is reported, so the change is visible rather than
 * mysterious.
 */
function planUploads(files, paths) {
  const planned = [];
  const skipped = [];
  ...
  for (const [index, file] of files.entries()) {
    ...
    if (parsed.error) {
      skipped.push({
        name: String(paths[index] || file.originalname || "(unnamed)"),
        reason: parsed.error
      });
      continue;
    }

    renamedFolders.push(...(parsed.adjusted || []));
    planned.push({ file, fileName: parsed.fileName, folderNames: parsed.folderNames });
  }

  return { planned, skipped, renamedFolders };
}

/* Find-or-create, so uploading into a tree that already has a "Notes" folder
 * adds to it rather than making a second one.
 */
function resolveFolderPath(organizer, parentId, names, created) {
  ...
  for (const name of names) {
    ...
    created.push(getFolderPath(organizer.folders, folder.id));
...
/* The files, into the folders that now exist.
 *
 * Unique within its own folder: two READMEs from two uploaded folders both
 * keep their names, which is the point of the whole layout.
 */
async function writeUploads(planned, savedOrganizer, skipped, deps) {
  const { markdownDir, ensureUniqueFilenameInDir, invalidateCachedContent } = deps;
  const written = [];
...
function folderUploadHandler(deps) {
  const { maxFolderDepth, readOrganizerState, mutateOrganizerState } = deps;
  ...
  return async (req, res, next) => {
    try {
      ...
      const paths = readUploadPaths(req, files);
      if (!paths) {
        res.status(400).json({ error: "Each uploaded file needs a matching entry in `paths`." });
        return;
      }

      // Where the picked folder is dropped: null means top level.
      const form = readBody(req, { parentId: { type: "string", nullable: true } });
      const parentId = form.parentId ? normalizeFolderId(form.parentId) : null;
      if (form.parentId && !parentId) {
        res.status(400).json({ error: "Invalid destination folder id" });
        return;
      }

      const { planned, skipped, renamedFolders } = planUploads(files, paths);
      if (planned.length === 0) {
      ...
      /* Folders first, then the files into them. The order is reversed from how
       * this used to work because a file's directory decides where it is, so
       * the directory has to exist before there is anywhere to write.
       */
      const createdFolderPaths = [];
      const { organizer: savedOrganizer } = await mutateOrganizerState((organizer) => {
        for (const entry of planned) {
          entry.folderId = resolveFolderPath(organizer, parentId, entry.folderNames, createdFolderPaths);
        }

        return null;
      });

      const written = await writeUploads(planned, savedOrganizer, skipped, deps);

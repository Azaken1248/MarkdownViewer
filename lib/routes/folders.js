...
function createFoldersRoutes({
  ...
  router.put("/api/folders/:folderId", requirePermission("doc:write"), async (req, res, next) => {
    try {
      ...
      // A parentId that is present and null means "to the top level"; one
      // that is absent means "leave it where it is". The reader keeps that
      // distinction: null comes through as null, absent is not a key at all.
      const body = readBody(req, {
        name: { type: "string" },
        parentId: { type: "string", nullable: true }
      });

...
function postLinks(deps) {
  const { linkStore } = deps;
...
// How many pages the icon sweep reads at once. Four, because the point is to
// be done before anybody looks rather than to be quick.
const ICON_BATCH_CONCURRENCY = 4;

/* The routes themselves: what this module is, in one screen. Each handler
 * that is more than a line is a function beside this one, taking the same
 * injected dependencies.
 */
function createLinksRoutes(deps) {
  const { linkStore, requireRead, requirePermission } = deps;
  ...
  router.post("/api/links", requirePermission("doc:write"), postLinks(deps));
   ...
   */
  router.post("/api/links/icons", requirePermission("doc:write"), postLinksIcons(deps));

  router.delete("/api/links/:id", requirePermission("doc:write"), deleteLinks(deps));

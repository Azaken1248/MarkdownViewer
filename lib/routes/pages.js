...
async function buildShareMeta(req, share, { getBaseUrl, markdownDir, readCachedTextFile }) {
...
// Addresses that are this app's own, and so are never a document's name.
...
function shareLinkPage(deps) {
  const { markdownDir, shareStore, fileExists, getShareTemplate, renderShareHtml, sendError } = deps;
  ...
  try {
    ...
    const meta = await buildShareMeta(req, share, deps);
...
function appShell(deps) {
  const { getIndexTemplate, buildEmbedMeta, renderIndexWithEmbedMeta } = deps;
...
function diagramPage(deps) {
  const { getDiagramTemplate } = deps;

  return async (req, res, next) => {
  try {
    // Read rather than sent as a file, because it is a template like the
    // others now: its asset versions are stamped in on the way out.
    res.set("Cache-Control", "no-cache").type("html").send(await getDiagramTemplate());
  } catch (error) {
    next(error);
  }
  };
...
/* The pages themselves: what this module is, in one screen. Each handler is a
 * function beside this one, taking the same injected dependencies.
 */
function createPagesRoutes(deps) {
  const { publicDir, assetVersions, serveStaticAsset, sendError } = deps;
  ...
  router.get("/s/:token", shareLinkPage(deps));
   ...
   */
  router.get(["/", "/index.html", "/links"], appShell(deps));
  ...
  /* The static files, and the one thing that makes 100 of them affordable.
   *
   * A request carrying the version the file actually has can be kept for a
   * year without asking again: the URL names those exact bytes, and different
   * bytes get a different URL. Anything else — a bare path, a stale version
   * somebody bookmarked — is revalidated as before, which is correct and only
   * costs a 304.
   *
   * Without this pairing the version does nothing at all. express.static's
   * default is `max-age=0`, so every one of the page's scripts and stylesheets
   * was a conditional GET on every load whether or not it carried a `?v=`.
   */
  /* The app's own scripts and stylesheets, compressed and cached in memory.
   * Anything it does not recognise — an image, a font, a missing file — falls
   * through to express.static below, which is still the one that answers.
   */
  router.use(serveStaticAsset);

  router.use(express.static(publicDir, {
    index: false,
    setHeaders(res, filePath) {
      const urlPath = `/${path.relative(publicDir, filePath).split(path.sep).join("/")}`;
      if (assetVersions.isCurrent(urlPath, res.req?.query?.v)) {
   ...
   */
  router.get(/^\/diagram\/(?:doc|file)\/.+$/, diagramPage(deps));

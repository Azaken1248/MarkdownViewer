...
// How far up a folder's parents go, stopping at the cap so a chain that loops
// cannot walk forever.
...
/* Break a cycle by detaching whichever folder closes it.
 *
 * Hand-edited state is the realistic source of these, and a cycle would hang
 * every walk over the tree.
 */
function breakCycleAbove(folder, byId) {
...
function sanitizeFolderHierarchy(folders) {
  ...
  // A parent that is the folder itself, or one that is not there at all.
 ...
 * to disagree.
 */
function createOrganizerFile({ filePath, db = null }) {
  const dataDir = path.dirname(filePath);
  const store = db || require("../db").open(dataDir);

  /* The organizer is the only record of which document lives in which folder.
   * Losing it is unrecoverable, so a state we cannot parse is treated as
   * "damaged, hands off" rather than "start fresh": reads degrade to no folder
   * info so documents stay browsable, and every write is refused until a human
   * intervenes. That rule held for the JSON file and it holds for the row.
   *
   * It also covers the one way the import can go wrong: a document-organizer.json
   * on disk that will not parse, with nothing yet in the database. Starting
   * fresh there would quietly discard somebody's folder layout and leave the
   * damaged file sitting beside a new one that knows nothing about it.
   */
  let organizerDamage = null;

  function importIfPresent() {
    if (!fs.existsSync(filePath)) {
      return;
    }

    const empty = store.prepare("SELECT COUNT(*) AS n FROM organizer").get().n === 0;
    if (!empty) {
      return;
    }
    ...
    let parsed;
    try {
      parsed = normalizeOrganizerState(JSON.parse(fs.readFileSync(filePath, "utf8")));
    } catch (parseError) {
      if (!organizerDamage) {
        console.error(
          `Organizer file could not be parsed (${parseError.message}). It has been left where it is. `
          + "Folder writes are disabled until data/document-organizer.json is valid JSON or removed."
        );
        organizerDamage = { backupPath: filePath, message: parseError.message };
      }
  ...
  function readUnsafe({ forWrite = false } = {}) {
    importIfPresent();

    if (organizerDamage && forWrite) {
      throw new HttpError(
        503,
        `Folder data on disk is damaged and was not overwritten (${path.basename(organizerDamage.backupPath)}). `
        + "Fix data/document-organizer.json, then retry."
      );
    }

    const row = store.prepare("SELECT state_json FROM organizer WHERE id = 1").get();
    if (!row) {
      // No row yet is a legitimate first run - not damage.
      return createDefaultOrganizerState();
    }

    try {
      return normalizeOrganizerState(JSON.parse(row.state_json));
    } catch (parseError) {
      // A row this process did not write correctly. There is no file to
      // quarantine; the row is left as it is and writes are refused, which is
      // the same answer the file got.
      if (forWrite) {
        throw new HttpError(503,
          `Folder data is damaged and was not overwritten (${parseError.message}). Repair the organizer row, then retry.`);
      }
  ...
  function writeUnsafe(organizerState) {
    const normalized = normalizeOrganizerState(organizerState);
    store.prepare(
      `INSERT INTO organizer (id, state_json, updated_at) VALUES (1, ?, ?)
       ON CONFLICT(id) DO UPDATE SET state_json = excluded.state_json, updated_at = excluded.updated_at`
    ).run(JSON.stringify(normalized), new Date().toISOString());
    return normalized;
  }

  async function readOrganizerState(options) {
    return readUnsafe(options);
  }

  /* Folder mutations are read-modify-write cycles. Every mutator this app
   * passes is synchronous, which is what lets the whole cycle be one SQLite
   * transaction: the read, the change and the write happen with the database
   * locked, so two requests — in this process or in another — cannot both read
   * the old state and have the second write silently discard the first.
   *
   * That is a stronger guarantee than the promise chain it replaces, which
   * serialised only within one process. It also means a mutator that awaited
   * would be a bug rather than a feature: better-sqlite3 cannot hold a
   * transaction across an await, and the check below says so early.
   */
  const mutate = store.transaction((mutator) => {
    const organizer = readUnsafe({ forWrite: true });
    const mutatorResult = mutator(organizer);
    if (mutatorResult && typeof mutatorResult.then === "function") {
      throw new Error("organizer mutators must be synchronous: the change runs inside a transaction");
    }

    const saved = writeUnsafe(organizer);
    return { organizer: saved, result: mutatorResult };
  });

  /* .immediate, and this is the line the multi-process guarantee rests on.
   *
   * A plain transaction begins deferred: it takes no lock until its first
   * write. This one reads first — the whole state — and only then writes, so
   * under WAL a deferred begin would read a snapshot, another process could
   * commit in the gap, and the write would then fail with BUSY_SNAPSHOT: the
   * snapshot is stale and no amount of waiting makes it current again. That is
   * exactly the lost-update the JSON file had, surfacing as an error rather
   * than silently. BEGIN IMMEDIATE takes the write lock before the read, so a
   * second process waits (busy_timeout) and then reads the state the first
   * one wrote.
   */
  async function mutateOrganizerState(mutator) {
    return mutate.immediate(mutator);
  }

  // Kept for the callers that still say it. What it stood for — one mutation
  // at a time — is now what every mutation gets from the transaction above.
  async function withOrganizerLock(task) {
    return task();
  }

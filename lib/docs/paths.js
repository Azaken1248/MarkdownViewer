...
// From the one file the client loads too. This is the only server module
// that reaches into public/, and it does so on purpose: what a document is
// has to be the same decision on both sides, and two copies of a list are two
// chances to disagree. See public/js/doc-kinds.js.
const DocKinds = require("../../public/js/doc-kinds");
const ALLOWED_DOC_EXTENSIONS = new Set(DocKinds.DOC_EXTENSIONS);
const { DOC_EXTENSIONS_SENTENCE } = DocKinds;
...
function sanitizeNewFilename(rawName) {
  ...
  // Any document extension keeps the name as given; only a bare name gets
  // .md. Wider than the editor's own rule (which will not take .ipynb, since
  // it writes text) because a name arriving here may be an upload of a real
  // notebook. Both decisions come from doc-kinds.js.
  return sanitizeFilename(DocKinds.isDocFile(trimmed) ? trimmed : `${trimmed}.md`);
}
...
function toDocTitle(fileName) {
  return DocKinds.stripDocExtension(docBaseOf(fileName))
    .replace(/[-_]+/g, " ")
    .replace(/\s+/g, " ")
    .trim()

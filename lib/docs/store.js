...
async function ensureStorageDirs(library) {
  await fsp.mkdir(library.markdownDir, { recursive: true });
  await fsp.mkdir(library.softDeletedDir, { recursive: true });
  await fsp.mkdir(library.hardDeletedDir, { recursive: true });
  await fsp.mkdir(library.dataDir, { recursive: true });
}
 ...
 */
async function migrateFlatLibraryToDirectories(library) {
  const organizer = await library.readOrganizerState();
  ...
  for (const dir of folderDirs) {
    await fsp.mkdir(path.join(library.markdownDir, dir), { recursive: true });
  }
  ...
  for (const [fileName, folderId] of Object.entries(mapping)) {
    ...
    const from = path.join(library.markdownDir, fileName);
    const to = path.join(library.markdownDir, dir, fileName);
  ...
  await library.mutateOrganizerState((state) => {
    state.fileFolders = {};
    return state;
  });
...
async function ensureUniqueFilename(library, filePath) {
  const dir = docDirOf(filePath);
  const unique = await ensureUniqueFilenameInDir(path.join(library.markdownDir, dir), docBaseOf(filePath));
...
async function moveDocToRecycle(library, fileName, mode) {
  const sourcePath = path.join(library.markdownDir, fileName);
  const recycleDir = mode === "hard" ? library.hardDeletedDir : library.softDeletedDir;
  ...
  library.invalidateCachedContent(sourcePath);
...
async function getRecycleDocs(library, organizerState = null, sourceDir = library.softDeletedDir) {
  const organizer = organizerState || await library.readOrganizerState();
...
async function getDocs(library, organizerState = null) {
  const organizer = organizerState || await library.readOrganizerState();
  const markdownEntries = (await walkDocs(library.markdownDir)).map((relative) => ({ name: relative }));
  ...
  const docs = await Promise.all(
    markdownEntries.map(async (entry) => {
      const fullPath = path.join(library.markdownDir, entry.name);
      const stat = await fsp.stat(fullPath);
 ...
 */
async function restoreFromBin(library, sourceDir, entryName) {
  ...
  const targetDir = path.join(library.markdownDir, docDirOf(originalFile));
  await fsp.mkdir(targetDir, { recursive: true });

  const restoreFileName = await ensureUniqueFilename(library, originalFile);
  const targetPath = resolveDocPath(restoreFileName, library.markdownDir);
  if (!targetPath) {
    return null;
  }
  ...
  await moveFile(sourcePath, targetPath);
  library.invalidateCachedContent(sourcePath);
  library.invalidateCachedContent(targetPath);
...
async function resolveNewDocumentPath(library, fileName, rawFolderId) {
...
/* The library: where the documents are, and how the organizer that files them
 * is reached.
 *
 * Every operation above takes it as its first argument rather than closing
 * over it, so each can be read — and tested — without the factory around it.
 * This is the wiring, and nothing else.
 */
function createDocumentStore(library) {
  return {
    ensureStorageDirs: () => ensureStorageDirs(library),
    migrateFlatLibraryToDirectories: () => migrateFlatLibraryToDirectories(library),
    fileExists,
    ensureUniqueFilenameInDir,
    ensureUniqueFilename: (filePath) => ensureUniqueFilename(library, filePath),
    moveFile,
    moveDocToRecycle: (fileName, mode) => moveDocToRecycle(library, fileName, mode),
    getRecycleDocs: (organizerState, sourceDir) => getRecycleDocs(library, organizerState, sourceDir),
    walkDocs,
    getDocs: (organizerState) => getDocs(library, organizerState),
    restoreFromBin: (sourceDir, entryName) => restoreFromBin(library, sourceDir, entryName),
    resolveNewDocumentPath: (fileName, rawFolderId) => resolveNewDocumentPath(library, fileName, rawFolderId)
  };
}

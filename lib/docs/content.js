...
function createDocumentCache({ contentMaxBytes, indexMaxBytes, snippetMaxBytes }) {
  ...
  return {
    ...
    // What the three of them are holding, for /metrics. The budgets are
    // constants somebody chose; this is how anybody finds out whether they
    // were the right ones.
    cacheStats: () => ({
      content: docContentCache.stats(),
      index: docSearchIndex.stats(),
      snippet: docSnippetCache.stats()
    })
  };
}

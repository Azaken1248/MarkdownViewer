...
/** What a page said about itself, and whether it could be read at all. A site
 * that would not answer still describes a link — with the hostname for a title
 * and the reason in `error` — because a card with a note on it is worth more
 ...
 */
/* The ranges an outward request must never reach, as a table rather than as a
 * ladder of ifs: the first octet, and where it matters the second. Written the
 * way the RFCs write them, so the list can be read against them.
 */
const BLOCKED_IPV4 = [
  { a: 0, why: "this network" },
  { a: 10, why: "private" },
  { a: 127, why: "loopback" },
  { a: 100, b: [64, 127], why: "carrier NAT" },
  { a: 169, b: [254, 254], why: "link-local, and cloud metadata" },
  { a: 172, b: [16, 31], why: "private" },
  { a: 192, b: [0, 0], why: "protocol assignments" },
  { a: 192, b: [168, 168], why: "private" },
  { a: 198, b: [18, 19], why: "benchmarking" },
  { a: 198, b: [51, 51], why: "documentation" },
  { a: 203, b: [0, 0], why: "documentation" }
];
...
function isBlockedIPv4(address) {
  ...
  return BLOCKED_IPV4.some((range) => range.a === a
    && (!range.b || (b >= range.b[0] && b <= range.b[1])));
}
...
function sendRequest(url, { accept, timeout, maxBytes, accepts, binary }) {
  return new Promise((resolve, reject) => {
    ...
    request.on("error", (/** @type {NodeJS.ErrnoException} */ error) => {
 ...
 *
 * The quoting matters more than it looks. `[^"']*` — read the value until
 * either kind of quote — truncates at the first apostrophe inside a
 * double-quoted value, and both of the things this file reads are full of
 * them: a description saying "Python's docs", and Cloudflare Access's inline
 * SVG icon, which is a double-quoted attribute containing `xmlns='...'`. The
 * value ended at the apostrophe and what came back was a fragment. So the
 * opening quote is captured and the value runs to the matching one.
 */
...
function metaContent(html, attribute, name) {
  ...
  for (const tag of html.match(/<meta\b[^>]*>/gi) || []) {
    if (attributeValue(tag, attribute).toLowerCase() !== wanted) {
      continue;
    }

    const content = collapse(attributeValue(tag, "content"));
    if (content) {
      return content;
    }
  }
 ...
 * draw a thumbnail of.
 */
/* One <link> tag, ranked: a real icon before an apple-touch-icon, and within
 * each, the size nearest 32px — bigger being better than smaller, because a
 * picture shown small is a picture and a picture shown big is a smudge.
 *
 * null for a tag that is not an icon at all.
 */
function rankIconTag(tag, base) {
  ...
  if (!rel.includes("icon") && !touch) {
    return null;
  }

  // "32x32", or "any" for an SVG, which is the best answer there is: one file
  // that is the right size at every size.
  ...
  return {
    rank: rel.includes("icon") ? 0 : 1,
    distance,
    href: resolveIconHref(collapse(href), base)
  };
}

function iconCandidates(html, pageUrl) {
  const head = String(html || "").slice(0, 400000);
  const base = baseUrlOf(head, pageUrl);
  const scored = [];

  for (const tag of head.match(/<link\b[^>]*>/gi) || []) {
    const scoredTag = rankIconTag(tag, base);
    if (scoredTag) {
  ...
  // is empty has nothing else to offer. Rooted at the origin rather than at
  // the base, because that is where it lives even on a page served under a
  // sub-path.
...
/* The address a relative href in this page is relative to.
 *
 * Nearly always the page's own address, but <base href> overrides it and
 * plenty of apps emit one — Grafana serves `<base href="/">` and then writes
 * its icon as `public/build/img/fav32.png` with no leading slash. Resolved
 * against the page that would be
 * /public-dashboards/<id>/public/build/img/fav32.png, which is a 404;
 * resolved against the base it is the icon. A browser reads the base tag, so
 * anything reading a page's addresses has to as well.
 */
function baseUrlOf(head, pageUrl) {
  const tag = head.match(/<base\b[^>]*>/i);
  const href = tag && attributeValue(tag[0], "href");
  ...
  try {
    return new URL(collapse(href), pageUrl);
  } catch {
    return pageUrl;
  }
}

/* An icon href as an absolute address, or as itself when it is already the
 * icon. A data: URI is left alone — there is nothing to resolve, and resolving
 * it against anything would only mangle it. */
 ...
 *
 * Each signature is asked the same question — does this buffer begin like
 * one of these — so the list reads as the list of formats this app shows.
 */
/** @type {[string, (bytes: Buffer) => boolean][]} */
const IMAGE_SIGNATURES = [
  ["image/png", (bytes) => bytes[0] === 0x89 && bytes.toString("latin1", 1, 4) === "PNG"],
  ["image/jpeg", (bytes) => bytes[0] === 0xff && bytes[1] === 0xd8 && bytes[2] === 0xff],
  ["image/gif", (bytes) => bytes.toString("latin1", 0, 3) === "GIF"],
  ["image/webp", (bytes) => bytes.toString("latin1", 0, 4) === "RIFF"
    && bytes.toString("latin1", 8, 12) === "WEBP"],
  // The .ico header: two zero bytes, then a type of 1 (icon) or 2 (cursor).
  ["image/x-icon", (bytes) => bytes[0] === 0 && bytes[1] === 0
...
function sniffImageType(buffer) {
  ...
  const known = IMAGE_SIGNATURES.find(([, looksLike]) => looksLike(buffer));
  if (known) {
    return known[0];
  }
  ...
  return /^<(\?xml|!doctype svg|svg)\b/i.test(start) && /<svg\b/i.test(start)
    ? "image/svg+xml"
    : "";
}
...
/* The bytes inside a data: URI, whichever way it was written.
 *
 * ;base64 is the form everyone expects, but the plain form is legal and used:
 * Cloudflare Access inlines its icon as a percent-encoded SVG, which is
 * shorter than base64 for text. Reading only one of the two spellings meant a
 * page that had handed over its icon still ended up with none.
 */
function decodeDataUri(value) {
  /* `[^,;]` rather than `[^,]` inside the repeat, which is not a nicety.
   *
   * With `[^,]` a parameter could itself contain `;`, so `;a;b` had two ways
   * to be read and a string of them had exponentially many. A page serving
   * `href="data:image/png;a;a;a…!"` — no comma, so the match must fail — made
   * this walk all of them: 24 of them took a quarter of a second, 40 would
   * take hours, and the href comes off a page a stranger asked us to preview.
   * A parameter that cannot hold the separator can only be read one way.
   */
 ...
 */
/* An icon inlined in the page. Nothing to fetch, and nothing to vet: it never
 * leaves this process.
 */
function inlineIcon(href) {
  const buffer = decodeDataUri(href);
  const type = buffer && sniffImageType(buffer);
  return type && buffer.length <= ICON_MAX_BYTES ? dataUri(type, buffer) : "";
}

/* One candidate, fetched and vetted.
 *
 * The address is already resolved against the page (or its <base>), and goes
 * through the same gate as the address that was typed — an icon href is an
 * address somebody else chose too, and "/favicon.ico" on a page that
 * redirected into the LAN is the whole attack again with a smaller file at
 * the end of it.
 */
async function fetchOneIcon(href, request) {
  let target;
  try {
    target = parseTarget(href);
  } catch {
    return null;
  }
  ...
  try {
    ...
    // At the cap means the read was cut off part-way, and half a PNG is not a
    // PNG — it would sniff as one and then fail to decode in the card.
    if (buffer.length === 0 || buffer.length >= ICON_MAX_BYTES) {
      return "";
    }

    const type = sniffImageType(buffer);
    return type ? dataUri(type, buffer) : "";
  } catch {
    return "";
  }
}
...
async function fetchIcon(pageUrl, html, request = requestIconOnce) {
  ...
  for (const href of iconCandidates(html, pageUrl)) {
    ...
    const fetched = await fetchOneIcon(href, request);
    // null is an address that could not even be parsed, which is not one of
    // the tries this is allowed.
    if (fetched === null) {
      continue;
    }
...
/* Just the icon, for a link that already has everything else.
 *
 * Separate from describeUrl because it is a different question. Re-reading a
 * page to get its icon would also replace its title and description with
 * whatever the page says today — including over a title someone corrected by
 * hand, which is the one thing editing a card is for.
 *
 * Never throws. Every failure — refused address, dead host, no icon on the
 * page — is the same answer: this link has been asked, and there is nothing to
 * show. That answer is worth storing, or it gets asked again on every visit.
 */
async function iconForUrl(rawUrl) {
  try {
    const url = parseTarget(rawUrl);
    const page = await fetchPage(url);
    return await fetchIcon(page.url, page.body);
  } catch {
    return "";
  }
}
 ...
 */
/** @param {string} rawUrl @returns {Promise<Preview>} */

...
function iconCandidates(html, pageUrl) {
  ...
  for (const tag of head.match(/<link\b[^>]*>/gi) || []) {
    ...
    scored.push({ rank, distance, href: resolveIconHref(collapse(href), base) });
  }
...
async function fetchIcon(pageUrl, html, request = requestIconOnce) {
  ...
  for (const href of iconCandidates(html, pageUrl)) {
    ...
    let target;
    try {
      // Already resolved against the page (or its <base>); put through the
      // same gate as the address that was typed, because an icon href is an
      // address someone else chose too — "/favicon.ico" on a page that
      // redirected into the LAN is the whole attack again with a smaller file
      // at the end of it.

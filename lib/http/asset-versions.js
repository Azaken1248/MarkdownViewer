...
// Only the app's own static assets — including /build, where `npm run build`
// puts the bundles that stand in for a page's scripts. Anything else on the
// page is a CDN URL with its own integrity hash, and rewriting those would
// break it.
const VERSIONED_URL = /^\/(?:css|js|build)\/[A-Za-z0-9._/-]+\.(?:css|js)$/;
...
const REFERENCE = /(\s(?:src|href)=")(\/(?:css|js|build)\/[^"?<>]+)(\?[^"<>]*)?(")/g;

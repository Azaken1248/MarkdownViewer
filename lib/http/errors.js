...
const { silentLog } = require("./logging");
...
function createErrorPages({
  ...
  const readErrorTemplate = templateReader(templatePath, stampAssetVersions);
  ...
  /** @param {{ heading?: string, message?: string, detail?: string, code?: string }} [override] */
  /** @typedef {{ heading?: string, message?: string, detail?: string, code?: string }} ErrorOverride */
  async function sendError(req, res, status, { heading, message, detail, code } = /** @type {ErrorOverride} */ ({})) {
    ...
    try {
      ...
      log.error("failed to render the error page", {
        id: req.id,
  ...
  function errorHandler() {
    return (error, req, res, _next) => {
      ...
      /* The stack goes to the log. The reader gets a status and nothing else:
       * an error page is not the place to publish internals.
       *
       * With the request's id on it, which is the whole point of having one —
       * this line and the access line above it are now the same request, and
       * the person reporting the failure has the id in their response headers.
       */

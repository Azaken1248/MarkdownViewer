...
//
// 'unsafe-inline' in style-src is a deliberate trade, and the reasons are
// three, not one — recorded here because a scanner flags it and somebody is
// asked to justify it:
//
//   1. This app's own drawing writes style="" attributes into the markup it
//      generates: a diagram box's fill and stroke come from the diagram's
//      classDef, and the folder picker's indent from a folder's depth. Those
//      values come from documents, so they cannot be hashed in advance, and
//      a nonce would not help — a nonce covers <style> elements, never a
//      style attribute. Moving them to the CSSOM after insertion is possible
//      and is a rewrite of the drawing path for a weaker vector than scripts.
//   2. KaTeX sets inline style attributes on what it typesets.
//   3. Mermaid injects a <style> block into every SVG it renders, with no
//      way to be told a nonce.
//
// Styles cannot execute; what style injection can do is redress the page.
// The headers suite pins the three reasons, so that the day all three are
// gone it fails and says to take this allowance out.
//

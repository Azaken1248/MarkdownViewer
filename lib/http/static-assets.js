 ...
 *
 * Only /css, /js and /build. Images and fonts are already compressed formats,
 * and running deflate over a PNG spends CPU to make it slightly bigger.
 */
...
const SERVED = /^\/(?:css|js|build)\/[A-Za-z0-9._/-]+\.(?:css|js)$/;
...
function createStaticAssets({ publicDir, assetVersions }) {
  ...
  // Every .css and .js under the directories this serves.
  function servedPaths() {
    const found = [];
    for (const top of ["css", "js", "build"]) {

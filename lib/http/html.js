...
// The same escaping the client uses, from the file that owns it, for the same
// reason paths.js takes its file extensions from doc-kinds.js: two copies of a
// rule about text are two rules. See public/js/dom-html.js.
const { escapeHtml } = require("../../public/js/dom-html");
 ...
 *
 * `transform` runs on the way out rather than on the way in, so what is cached
 * is the file and not the result. Asset versions are stamped through it, and
 * those change when an asset changes — which is a different file's mtime, and
 * one this cache knows nothing about.
 */
function templateReader(filePath, transform = null) {
  let cached = null;
  let cachedMtimeMs = 0;

...
/** @param {string} password @param {Buffer} salt @param {{ N?: number, r?: number, p?: number }} params */
async function derive(password, salt, params) {
  // @ts-ignore — the promisified scrypt takes the options object node's own
  // callback form takes; @types/node only describes the callback overloads.

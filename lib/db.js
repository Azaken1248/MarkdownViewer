...
const SCHEMA = `
  ...
  -- The search index. FTS5 with the trigram tokenizer, which is what keeps
  -- the search box meaning what it meant: a query matches any substring of
  -- three characters or more, case-insensitively, exactly as the scan over
  -- every document's lowercased text did — only now the documents that match
  -- come back from the index rather than every document being read to find
  -- out. The body is stored here too, so a match arrives with its text and
  -- no file is opened to score or snippet it. Scope keeps the library, the
  -- recycle bin and the archive apart.
  ...
  -- What the index holds, so a search can tell what changed since. The walk
  -- that lists the documents already carries each one's mtime and size; a row
  -- here that disagrees is a document to re-read, a document with no row is
  -- new, and a row with no document is gone. That is how a file edited or
  -- moved outside this app stays searchable without watching the disk.
  --
  -- The id is the FTS row's rowid, and every delete from the FTS table goes by
  -- it. scope and file are UNINDEXED there — an FTS5 table indexes text, not
  -- keys — so a delete by those columns scans the whole index, and a reconcile
  -- that did that per document was quadratic: sixty seconds for five thousand.
  ...
  -- Failed sign-ins, per account and per address, and the lockout they earn.
  -- In the database rather than in memory so that a restart does not reset
  -- the count — a crash loop or a redeploy used to turn the lockout into a
  -- speed bump — and so that two processes share one count rather than each
  -- allowing the full number of guesses. This is the one limit in the app
  -- that is a security boundary rather than a ceiling, which is why it is the
  -- one that lives here and the others do not.
  CREATE TABLE IF NOT EXISTS login_attempts (
    key             TEXT PRIMARY KEY,
    -- Timestamps of recent failures, as a JSON array. At most forty.
    attempts_json   TEXT NOT NULL DEFAULT '[]',
    locked_until    REAL NOT NULL DEFAULT 0,
    last_attempt_at REAL NOT NULL
  );
  CREATE INDEX IF NOT EXISTS login_attempts_last ON login_attempts (last_attempt_at);
...
/* Switch the file to WAL, and keep asking if told it is busy.
 *
 * The switch takes an exclusive lock, and on that one transition SQLite does
 * not consult the busy handler: two processes opening the same brand-new file
 * at the same moment — a cluster starting its workers — can leave one with
 * SQLITE_BUSY straight from the pragma, timeout or no timeout. Measured at
 * about one open in a thousand, which is one worker in a thousand boots that
 * dies before the schema. Once any process has made the switch the file is
 * WAL for good and the pragma is a read that cannot be refused, so a retry
 * only ever matters in that first instant, and it waits a few milliseconds
 * for it.
 */
const WAL_RETRIES = 50;
const WAL_RETRY_MS = 20;
...
function enableWal(db) {
  for (let attempt = 1; ; attempt += 1) {
    try {
      ...
      throw new Error(`journal_mode is ${mode}, not wal`);
    } catch (error) {
      if (error.code !== "SQLITE_BUSY" || attempt >= WAL_RETRIES) {
        throw error;
      }
...
function open(dataDir) {
  ...
  // First, before anything that takes a lock. The switch to WAL below takes
  // one, and with the default timeout of zero a second process opening the
  // same new file at the same moment — a cluster starting its workers — gets
  // SQLITE_BUSY from that pragma and never reaches the schema. The db suite
  // runs three processes against one fresh database at once and this is
  // where one of them died, on one machine in ten. A wait here is what the
  // timeout is for.
